...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            x1 = batch['node1_features'].to(device)
            x2 = batch['node2_features'].to(device)
            edge_index_1 = batch['edge_index_1'].to(device)
            edge_index_2 = batch['edge_index_2'].to(device)
            mapping_1 = batch['mapping_1'].to(device)
            mapping_2 = batch['mapping_2'].to(device)
            labels = batch['label'].to(device)

            pred_1, pred_2, _ = model(x1, edge_index_1, x2, edge_index_2)
            inputs_embeds_1 = pred_1[mapping_1]
            inputs_embeds_2 = pred_2[mapping_2]

            combined_pred = torch.cat([inputs_embeds_1, inputs_embeds_2], dim=-1)
            loss = loss_func(combined_pred, labels)
            loss.backward()
        ...
        with torch.no_grad():
            for step, batch in enumerate(val_loader):
                ...
                # Convert predictions to binary labels and compare with actual labels
                predicted_labels = torch.argmax(combined_pred, dim=1)
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_loader):
            ...
            # Convert predictions to binary labels for comparison
            predicted_labels = torch.argmax(combined_pred, dim=1)
            correct += (predicted_labels == labels).sum().item()
